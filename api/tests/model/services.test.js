import configureApp from '../../config/init';
import Service from '../../model/service';

describe('Service Model', () => {
  let disconnectDB;

  beforeEach(async () => {
    disconnectDB = (await configureApp())[1];
  });

  beforeEach(async (done) => {
    await Service.deleteMany({});
    done()
  });

  afterAll(async (done) => {
    await Service.deleteMany({});
    await disconnectDB();
    done()
  });

  const validService = {
    name: 'test',
    description: 'description',
    banner: "",
    type: "hair",
    subtype: "dyes",
    price: 9.99,
    addons: [
      {
        name: "More color",
        price: 9.99
      }
    ],
  }

  it ('should be valid with all valid parameters', async () => {
    expect(await Service.validate(validService)).toEqual(undefined);
  });

  it ('should be not valid with an invalid name', async (done) => {
    try {
      await Service.validate({ ...validService, name: undefined })
      done("Failed to throw validation error")
    } catch(err) {
      done()
    }
  });

  it ('should be not valid with an invalid description', async (done) => {
    try {
      await Service.validate({ ...validService, description: undefined })
      done("Failed to throw validation error")
    } catch(err) {
      done()
    }
  });

  it ('should be not valid with an invalid banner', async (done) => {
    try {
      await Service.validate({ ...validService, banner: undefined })
      done("Failed to throw validation error")
    } catch(err) {
      done()
    }
  });

  it ('should be not valid with an invalid type', async (done) => {
    try {
      await Service.validate({ ...validService, banner: undefined })
      done("Failed to throw validation error")
    } catch(err) {
      done()
    }
  });

  it ('should be not valid with an invalid type', async (done) => {
    try {
      await Service.validate({ ...validService, banner: undefined })
      done("Failed to throw validation error")
    } catch(err) {
      done()
    }
  });

  it ('should be valid with a missing subtype', async () => {
    expect(await Service.validate({ ...validService, subtype: undefined })).toEqual(undefined);
  });

  it ('should be valid with a missing price', async () => {
    expect(await Service.validate({ ...validService, price: undefined })).toEqual(undefined);
  });

  it ('should be valid with missing addons', async () => {
    expect(await Service.validate({ ...validService, addons: undefined })).toEqual(undefined);
  });
});