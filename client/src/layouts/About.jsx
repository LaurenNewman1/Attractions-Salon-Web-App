...
import useStaff from '../stores/AboutStore';
import Loading from '../components/Loading';

const defaultBio = 'No information available.';
...
const About = () => {
  ...
  return (
    <Page maxWidth={false}>
      ...
      <Grid container spacing={4} className={classes.teamContainer}>
        ...
        {staff.map((member) => (
          <Grid item xs={6} sm={4} md={3} style={{ textAlign: 'center' }}>
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <Avatar alt="" src={member.banner} className={classes.avatar} />
            </div>
            <Typography variant="h6" color="primary">{member.name}</Typography>
            <Typography variant="subtitle1">{member.title}</Typography>
            <Typography variant="body2" color="textSecondary">{member.bio || defaultBio}</Typography>
          </Grid>
        ))}
      </Grid>
    </Page>
